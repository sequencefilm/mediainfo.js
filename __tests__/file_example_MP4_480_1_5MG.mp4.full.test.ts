...
it('should return full data', async () => {
  ...
  expect(track0.FileSize_String).toBe('1.50 MiB')
  expect(track0.Duration_String).toBe('30 s 527 ms')
  expect(track0.OverallBitRate_String).toBe('411 kb/s')
  expect(track0.StreamSize_String).toBe('11.6 KiB (1%)')
  ...
  expect(track1.StreamSize_String).toBe('1.08 MiB (72%)')
